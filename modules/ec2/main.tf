locals {
  ...
  #   for index, value in var.ec2_instances : [
  #     for ebscount in range(value.ebs-volume-count) : {
  #       az        = value.availability-zone
  #       ebs-count = value.ebs-volume-count
  #       ins-count = value.instance-count
  #       key       = index
  #       size      = value.ebs-volume-size
  #     }
  #   ]
  # ])
...
data "aws_ami" "info" {
  for_each = { for index, v in var.ec2_instances : index => v }
  filter {
    name   = "image-id"
    values = [lookup(each.value, "ami", "ami-0a481e6d13af82399")]
  }

  owners = [lookup(each.value, "ami-owner", "137112412989")]
}

resource "aws_instance" "default" {
  for_each                    = { for key, v in var.ec2_instances : key => v }
  ...
  iam_instance_profile        = lookup(each.value, "iam-instance-profile-name", null)
